...
from Recommendation_taobao_Model import Crop, Mask

tf.compat.v1.disable_eager_execution()
tf.random.set_seed(42)


def data_partition_TAOBAO(fname):
    usernum = 0
    itemnum = 0
...
def evaluate(model, dataset, args, sess):
    ...
    HT = 0.0
    valid_user = 0.0
    from Recommendation_taobao_index import tstUsrs
    if usernum > 10000:
...
def Model_pro(in_seq, mask, seq_cxt, args, buy_seq_mask, cart_seq_mask, fav_seq_mask, click_seq_mask, resweight_buy,
    ...
    # seq = normalize(buy_seq) + normalize(cart_seq) + normalize(fav_seq) + normalize(click_seq)  +seq_cxt_emb
    # seq = normalize(seq) + t
    # seq= seq_res
    # seq = seq + t
...
def Augmented_seq(seq, beh_seq, behavior, seq_w, maxlen):
    ...
    from Recommendation_taobao_index import args

...
class Model():
    def __init__(self, usernum, itemnum, args, reuse=None):
        ...
        trgt_cxt_emb = tf.compat.v1.layers.dense(inputs=self.pos_cxt, units=args.hidden_units, activation=None,
                                                 reuse=True,
        ...
        # cxt
        pos_emb_in = tf.compat.v1.layers.dense(inputs=pos_emb, reuse=True, units=args.hidden_units, activation=None,
                                               kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                               name="feat_emb")
        neg_emb_in = tf.compat.v1.layers.dense(inputs=neg_emb, reuse=True, units=args.hidden_units, activation=None,
                                               kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                               name="feat_emb")

        seq_emb = tf.reshape(self.seq, [tf.shape(self.input_seq)[0] * args.maxlen, args.hidden_units])
        Aug_seq_emb = tf.reshape(self.Aug_seq, [tf.shape(self.input_seq)[0] * args.maxlen, args.hidden_units])
        pos_emb = tf.reshape(pos_emb_in, [tf.shape(self.input_seq)[0] * args.maxlen, args.hidden_units])
        neg_emb = tf.reshape(neg_emb_in, [tf.shape(self.input_seq)[0] * args.maxlen, args.hidden_units])
        ...
        self.classification_pred = tf.compat.v1.layers.dense(inputs=classification_pred1, units=4,
        ...
        self.final_loss = self.loss + 0.15 * self.classification_loss  # best is 0.15
    ...
    def predict(self, sess, u, seq, item_idx, seq_cxt, test_item_cxt, buy_seq_mask, cart_seq_mask, fav_seq_mask,
                click_seq_mask):
        from Recommendation_IJCAI_index import model
...
class WarpSampler(object):
    def __init__(self, User, Beh, Beh_w, Behaviors, usernum, itemnum, batch_size=64, maxlen=10, n_workers=1):
        ...
        self.processors = []
        from Recommendation_IJCAI_Method import sample_function
...
class Substitute(object):
    ...
    def __init__(self, substitute_rate=0.1):
        from Recommendation_IJCAI_Method import similarity_dict
    ...
    def __call__(self, sequence):
        ...
        for index in substitute_idx:
            # if self.ensemble:
            # substitute_items = most_similar(self.similarity_dict, copied_sequence[index])
            substitute_items = similarity_dict[copied_sequence[index]]

...
def Augmented_seq(seq, beh_seq, behavior, seq_w, maxlen):
    ...
    from Recommendation_IJCAI_index import args
    from Recommendation_IJCAI_Model import Substitute
    if len(seq) < args.min_seq:
        new_seq = seq
    else:  # if the sequence will be augmented then we check the neighbours and send the neighbor sequece instead, if there are no neighbours we crop the original seq.
        # print('reading similarity...')
        substitute = Substitute(substitute_rate=0.0)
        # print('substitute...')
        new_seq = substitute(seq)
        # new_beh_seq, new_behavior, new_seq_w = beh_seq, behavior, seq_w
    ...
    for i in reversed(new_seq[:]):
        ...
        Aug_seq[idx] = i
        '''
        if new_behavior[seq_len-1]== 'buy':
          Aug_buy_seq_mask[idx] = 1
        elif new_behavior[seq_len-1] == 'cart':
          Aug_cart_seq_mask[idx] = 1
        elif new_behavior[seq_len-1] == 'fav':
          Aug_fav_seq_mask[idx] = 1
        elif new_behavior[seq_len-1] == 'pv':
    ...
    '''
    # print('Augmented sequence done....', Aug_seq)
...
def sample_function(user_train, Beh, Beh_w, Behaviors, usernum, itemnum, batch_size, maxlen, result_queue, SEED):
    def sample():
        ...
        for i in reversed(user_train[user][:-1]):
            ...
            if Behaviors[user][seq_len - 1] == 'buy':
                buy_seq_mask[idx] = 1
            elif Behaviors[user][seq_len - 1] == 'cart':
                cart_seq_mask[idx] = 1
            elif Behaviors[user][seq_len - 1] == 'fav':
                fav_seq_mask[idx] = 1
            elif Behaviors[user][seq_len - 1] == 'pv':
        ...
        # pos_weight = list()
        # neg_weight = list()
        seq_len_cxt = len(user_train[user]) - 1
        # print('................................. input....................................')
        # print('sequence length is....', user_train[user])
        # print('sequence weight ................', Beh_w[user])
        # print('sequence behaviour is.... ', Beh[user])
        # print(' behaviours is.... ', Behaviors[user])

        # print('................................. seq....................................')
        # print('sequence length is....', seq)

        for i in seq:
            if i == 0:
                seq_cxt.append([0, 0, 0, 0])
            else:
                seq_cxt.append(Beh[user][seq_len_cxt - 1])
                seq_len_cxt -= 1
        # print('................................. seq_cxt ....................................')
        # print(' ....', seq_cxt)

        # print(' behaviours is.... ', len(Behaviors[user]))
        # print('................................. pos ....................................')
        # print(' ....', pos)
        ...
        for i in pos:
            ...
            if i == 0:
                ...
                if Behaviors[user][pos_seq_len] == 'buy':
                    labels.append([1, 0, 0, 0])

                elif Behaviors[user][pos_seq_len] == 'cart':
                    labels.append([0, 1, 0, 0])

                elif Behaviors[user][pos_seq_len] == 'fav':
                    labels.append([0, 0, 1, 0])

                elif Behaviors[user][pos_seq_len] == 'pv':
        ...
        # Aug_seq, Aug_seq_cxt, Aug_buy_seq_mask, Aug_cart_seq_mask, Aug_fav_seq_mask, Aug_click_seq_mask  = Augmented_seq(user_train[user][:-1],Beh[user][:-1],Behaviors[user][:-1], Beh_w[user][:-1],maxlen)

        Aug_seq = Augmented_seq(user_train[user][:-1], Beh[user][:-1], Behaviors[user][:-1], Beh_w[user][:-1], maxlen)
        return (
            user, seq, pos, neg, seq_cxt, pos_cxt, pos_weight, neg_weight, buy_seq_mask, cart_seq_mask, fav_seq_mask,
            click_seq_mask, recency, \
            Aug_seq, seq_cxt, buy_seq_mask, cart_seq_mask, fav_seq_mask, click_seq_mask, labels)
...
def data_partition_IJCAI(fname):
    usernum = 0
    itemnum = 0
    interactions = 0
...
def evaluate(model, dataset, args, sess):
    ...
    from Recommendation_IJCAI_index import tstUsrs
    NDCG = 0.0
    HT = 0.0
    valid_user = 0.0
...
def evaluate_valid(model, dataset, args, sess, Beh, epoch):
    ...
    for u in users:
        ...
        for i in reversed(train[u]):
            ...
            if Behaviors[u][seq_len] == 'buy':
                buy_seq_mask[idx] = 1
            elif Behaviors[u][seq_len] == 'cart':
                cart_seq_mask[idx] = 1
            elif Behaviors[u][seq_len] == 'fav':
                fav_seq_mask[idx] = 1
            elif Behaviors[u][seq_len] == 'pv':
                click_seq_mask[idx] = 1
...
similarity_path = './Datasets/Tianchi_best_item_cf_iuf_similarity.pkl'
similarity_dict = load_similarity_dict(similarity_path)  # to load the sim dict only once


def most_similar(similarity_dict, item, top_k=8):
    # print('Length of items in dict: ',len(similarity_dict.keys()))
    top_k_items_with_score = sorted(similarity_dict[item].items(), key=lambda x: x[1], \

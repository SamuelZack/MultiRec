...
import timeit
from Recommendation_IJCAI_Method import data_partition_IJCAI, evaluate_valid
from Recommendation_IJCAI_Model import WarpSampler, Model, Args
...
print('maxlen...', args.maxlen, '   heads....', args.num_heads, '   dropout...', args.dropout_rate, '  lr,...', args.lr,
      ' emb size...', args.hidden_units, 'min len..', args.min_seq)
print(' augmentation only crop 0.2 and mask 0.4 .... loss weight 0.02')
dataset = data_partition_IJCAI(args.dataset)
print('here')
...
for epoch in range(1, args.num_epochs + 1):
    ...
    for step in range(0, int(num_batch)):
        ...
        loss, _, class_loss, classification_pred = sess.run(
            [model.final_loss, model.train_op, model.classification_loss, model.classification_pred],
            {model.u: u, model.input_seq: seq, model.pos: pos, model.neg: neg, model.buy_seq_mask: buy_seq_mask,
             model.cart_seq_mask: cart_seq_mask,
             model.fav_seq_mask: fav_seq_mask, model.click_seq_mask: click_seq_mask, model.is_training: True,
             model.seq_cxt: seq_cxt, model.pos_cxt: pos_cxt, model.pos_weight: pos_weight,
             model.neg_weight: neg_weight, model.recency: recency,
             model.Aug_input_seq: Aug_seq, model.Aug_seq_cxt: Aug_seq_cxt, model.Aug_buy_seq_mask: Aug_buy_seq_mask,
             model.Aug_cart_seq_mask: Aug_cart_seq_mask,
             model.Aug_fav_seq_mask: Aug_fav_seq_mask, model.Aug_click_seq_mask: Aug_click_seq_mask,
             model.labels: labels})
        # print('input sequence....', seq)
        # print('predictions.......', classification_pred)
        # print('loss.......', class_loss)
        # print(abc)
        total_loss = total_loss + loss
        class_total_loss = class_total_loss + class_loss
        stop = timeit.default_timer()
        # print('Time for batch in sec.....: ', stop - start)
    # print(abc)
    print('loss in epoch...', epoch, ' is  ', total_loss / int(num_batch))
    print('classification loss in epoch...', epoch, ' is  ', class_total_loss / int(num_batch))
    if epoch % 5 == 0:
        # print(abc)
        t1 = time.time() - t0
        T += t1
